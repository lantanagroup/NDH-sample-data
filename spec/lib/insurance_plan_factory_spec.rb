...
RSpec.describe PDEX::InsurancePlanFactory do
  ...
  describe '#build' do
    ...
    it 'includes an id' do
      expect(resource.id).to eq("ndh-insuranceplan-#{plan.id}")
    end
    ...
    it 'includes an owner reference' do
      expect(resource.ownedBy.reference).to eq("Organization/ndh-organization-#{plan.owner_id}")
      expect(resource.ownedBy.display).to eq(plan.owner_name)
    end

    it 'includes an administrator reference' do
      expect(resource.administeredBy.reference).to eq("Organization/ndh-organization-#{plan.administrator_id}")

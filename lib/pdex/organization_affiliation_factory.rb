...
module PDEX
  class OrganizationAffiliationFactory
    ...
    def id
      "ndh-organizationaffiliation-#{source_data.npi}"
    end
    ...
    def part_of_id
      "ndh-organization-#{managing_org.part_of_id}"
    end

    def organization
      {
        reference: "Organization/ndh-organization-#{managing_org.npi}",
        display: managing_org.name
      }
    end
    ...
    def participatingOrganization
      {
        reference: "Organization/ndh-organization-#{source_data.npi}",
    ...
    def network
      networks.map do |network_data|
        {
          reference: "Organization/ndh-network-#{network_data.npi}",

    ...
    def load
      ...
      def load_managing_organizations
        CSV.foreach(MANAGING_ORG_FILENAMES, headers: true) do |row|
          if insurance_plan? row
            NPPESDataRepo.plans << NDH::NPPESManagingOrg.new(row)
          elsif managing_org? row
            NPPESDataRepo.managing_orgs << NDH::NPPESManagingOrg.new(row, managing_org: true)
          elsif payer? row
            NPPESDataRepo.payers << NDH::NPPESManagingOrg.new(row, payer: true)
          end
        end
      end
      ...
      def load_networks
        CSV.foreach(NETWORK_FILENAMES, headers: true) do |row|
          NPPESDataRepo.networks << NDH::NPPESNetwork.new(row)
        end
      end

      def load_organizations
        CSV.foreach(ORGANIZATION_FILENAMES, headers: true) do |row|
          NPPESDataRepo.organizations << NDH::NPPESOrganization.new(row)
        end
      end

      def load_practitioners
        CSV.foreach(PRACTITIONER_FILENAMES, headers: true) do |row|
          NPPESDataRepo.practitioners << NDH::NPPESPractitioner.new(row)
        end
      end

      def load_pharmacies
        CSV.foreach(PHARMACY_FILENAMES, headers: true) do |row|
          NPPESDataRepo.pharmacies << NDH::PharmacyData.new(row)
        end
      end
      ...
      def load_pharmacy_orgs
        ...
        unique_org_names = NPPESDataRepo.pharmacies.map{ |pharmacy| short_name(pharmacy.name)}.uniq.sort 
         unique_org_names.each {|name| 
          NPPESDataRepo.pharmacy_orgs << NDH::PharmacyOrgData.new(name)
        }
      end
    end
  end

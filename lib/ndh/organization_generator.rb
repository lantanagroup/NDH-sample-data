...
module NDH
  class OrganizationGenerator
   ...
   def initialize(nppes_data)
    ...
    def organization
      @organization ||= NDH::OrganizationFactory.new(nppes_data).build
    end

    def organization_affiliation
      NDH::OrganizationAffiliationFactory.new(
        nppes_data,
        networks: networks,
    ...
    def state_networks
      NDH::NPPESDataRepo.networks_by_state(state)
    end

    def networks
      @networks ||= state_networks.sample(nppes_data.name.length % state_networks.length + 1)
      NDH::NPPESDataRepo.organization_networks[nppes_data.npi] = @networks
    end

    def state_managing_orgs
      NDH::NPPESDataRepo.managing_orgs_by_state(state)
    end
